...
class SensorModel:
    ...
    def __init__(self, occupancy_map, params):
        ...
        ### Start adding the params as we do implementation ###
        ...
        ### Params for 4 probability distributions
    ...
    def p_hit(self, z_tk, x_t, z_tk_star):
        if 0 <= z_tk <= self.z_max:
            gaussian = (math.exp(-(z_tk - z_tk_star)**2 / (2 * self.var_hit**2)))/ math.sqrt(2 * math.pi * self.var_hit**2)
            return gaussian
        else:
            return 0.0
    ...
    def rayCast_legacy(self, measurements, x_t1, occupancy_map):
        ...
        x, y, theta = x_t1
        laser_coord_x, laser_coord_y = x+(self.laser_sensor_offset)*np.cos(theta), y+(self.laser_sensor_offset)*np.sin(theta)
        ...
        for idx, heading in enumerate(rays_counterclockwise):
            ...
            ray_succeeded = np.array([self.ray_step_size * np.cos(heading),
                                      self.ray_step_size * np.sin(heading)]) # a pair of unit proceedings in x and y directions            
